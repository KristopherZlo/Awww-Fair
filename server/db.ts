...
export function createMigrationStatements(): string[] {
  return [
    `CREATE TABLE IF NOT EXISTS users (
      ...
      two_factor_recovery_hashes JSON NULL,
    ...
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS avatar_shape VARCHAR(16) NOT NULL DEFAULT 'circle' AFTER avatar_url`,
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS two_factor_enabled BOOLEAN NOT NULL DEFAULT FALSE AFTER email`,
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS two_factor_secret VARCHAR(64) NULL AFTER two_factor_enabled`,
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS two_factor_pending_secret VARCHAR(64) NULL AFTER two_factor_secret`,
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS two_factor_recovery_hashes JSON NULL AFTER two_factor_pending_secret`,
